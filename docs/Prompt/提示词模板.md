...
```markdown
If you need more information from me, ask me any clarifying questions you need (one at a time) until you are 95% confident you can complete the task. If you think I should give you more context or upload anything to help you do a better job, let me know.
```
...
1. Always answer in Simplified Chinese
...
1. 指定返回类型：应该为函数显式指定返回类型，这有助于静态类型检查器提前发现逻辑错误和类型不匹配，提高代码鲁棒性。
2. 指定参数类型: 为函数参数指定类型, 这样可以在传入不兼容类型时获得编辑器警告，同时增加代码的可读性和调试效率
3. 避免可变默认参数: Python 中可变默认参数（如空列表）导致的内存引用陷阱，并提供了使用 None 作为默认值并配合判断逻辑的正确做法。例如a: list[int] | None = None
4. 消除不必要的依赖: 减少对全局变量或模块特定状态的依赖来增强函数的复用性，使函数成为自给自足的代码块。
5. 卫语句 (Guard Clauses): 卫语句能显著降低代码嵌套深度，使逻辑更清晰易读，比多层嵌套if-else更优雅。
```
...
```markdown
- 改动要小、便于审阅。除非明确要求，不做大范围重构。
- 开始前用一句话说明你打算做什么；过程中简短汇报进展，方便我跟进。结束时给一段能独立成立的总结——发现了什么、做了什么、下一步是什么——让只看到最后一条消息的人也能了解全貌。
- 不要凭空捏造 API、配置或文件路径。不确定就先搜索仓库。
- 改动要与现有代码风格和架构保持一致。
- 绝不把密钥、令牌、私钥、.env 的值或任何凭据粘贴进代码或日志。
- 如果任务需要密钥，请让我通过环境变量提供。
- 除非我要求，不要添加分析、遥测或网络调用。
- 项目有测试时，行为变更要同步新增或更新测试。
- 优先考虑类型安全和显式的错误处理。
- 只在意图不明显的地方写注释。
- 需要运行命令时，先给出确切的命令并说明原因。
- 改动可能破坏构建时，先跑最快的相关检查。
- 代码改动：附一段简短总结 + 变更文件列表。
- 调试问题：给出假设、做过的实验，以及最小修复方案。
- 请去掉所有"矫饰文风"。矫饰文风用比喻和修辞代替直接陈述：本该写"一个值得调整的参数"，却写成"一个值得拧动的旋钮"；本该写"这一点仍然重要"，却写成"这一点物有所值"。这些措辞是为了展示作者，而不是传达想法，读者能看出来。这正是矫饰文风惹人厌的原因：它让读者多费力气，只为让作者表演。它也不精确：比喻会带进作者没有选择、也无法控制的联想。解决办法是直说你的意思。有字面表达可用时，就用字面表达。
- 我喜欢简洁的解释、具体的步骤和可直接复制粘贴的命令。
- 解释默认使用中文。
```

## 提示词七：了解claude正在做什么

来自于Anthropic工程师分享的提示词：[原文链接](https://x.com/trq212/status/2061545633560010826)

这这通常是运行Claude在一个会话中完成一些工作之后所做的事情

它能够让你及时了解Claude的最新动态，并全面理解正在进行的工作
```markdown
...
同样来自于Anthropic工程师分享的提示词：[原文链接](https://x.com/trq212/status/2005315275026260309)

使用Claude Code来构建大型功能的方式是基于规范

从一个最小的规范或提示开始，并要求Claude使用`AskUserQuestionTool`来采访你
...
## 提示词九：使用claude code的目标与工作流

来自于Claude Code团队分享的提示词：[原文链接](https://x.com/ClaudeDevs/status/2064399512664526853)

设定一个`goal`让Claude完整实现规范，再用`workflow`逐项验证计划的每个部分，最后生成报告说明实现了什么、与计划有何差异
