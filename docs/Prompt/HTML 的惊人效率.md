...
>
> 原文推文：https://x.com/trq212/status/2052809885763747935
>
> 用例演示页面：https://thariqs.github.io/html-effectiveness/
...
HTML 是 Claude 深入分析问题的富画布。当你开始研究一个问题时，不要只做一个简单的 Markdown 计划，而是让 Claude 生成一系列 HTML 文件。例如：先让 Claude 头脑风暴并生成多个方案探索，然后选择一个深入展开，制作原型或代码片段。最后，让它编写实施计划。当你满意后，开一个新会话，把所有这些文件传入让它实现。

验证时，你也可以让验证代理读取这些文件，它将拥有更广泛的上下文来了解需要什么。

### 示例提示词

```
我不确定引导页（onboarding screen）该往哪个方向做。
生成 6 种截然不同的方案 —— 在布局、语气和信息密度上做出差异化
—— 然后把它们排列在一个 HTML 文件的网格中，让我可以并排比较。
给每个方案标注它所做的取舍。
```

```
创建一个详尽的实施计划，输出为 HTML 文件。确保包含一些原型示意图、
数据流展示，以及我可能需要审阅的关键代码片段。让它易于阅读和理解。
```
...
代码在 Markdown 文件中可能很难阅读，但通过 HTML 我们可以渲染 diff、注释、流程图、模块等。用它来理解代理编写的代码，进行代码审查，或者向审查你代码的人解释 PR。这通常比默认的 GitHub diff 视图效果更好，原文作者现在每个 PR 都会附带一个 HTML 代码解释器。

### 示例提示词

```
帮我审查这个 PR，创建一个描述它的 HTML 产物。
我对流式处理/背压（streaming/backpressure）逻辑不太熟悉，所以重点关注这部分。
渲染实际的 diff 并附上行内边注，按严重程度对发现的问题进行颜色编码，
以及其他任何有助于清晰传达概念的方式。
```
...
Claude Design 基于 HTML，因为 HTML 在设计表达上非常强大，即使你的最终产物不是 HTML。Claude 可以先用 HTML 草拟设计，然后用你选择的语言实现，无论是 React、Swift 等。

你还可以为交互进行原型设计，比如动画、操作等。考虑让 Claude 制作滑块、旋钮等，精确调整你想要的效果。

### 示例提示词

```
我想为一个新的结账按钮做原型，点击后播放一个动画然后快速变为紫色。
创建一个 HTML 文件，包含多个滑块和选项让我尝试这个动画的不同参数，
给我一个复制按钮来复制效果好的参数组合。
```
...
Claude Code 非常擅长综合来自多个数据源的信息，并将其转换为可读性强的报告。你可以让 Claude 搜索你的 Slack、代码库、git 历史、互联网等，并用它为自己、领导层、团队等生成极具可读性的报告。

你可以将其组装成长 HTML 文档、交互式解释器甚至幻灯片/演示文稿。让 Claude 使用 SVG 制作图表来帮助可视化。

### 示例提示词

```
我不理解我们的速率限制器到底是怎么工作的。阅读相关代码，
生成一个 HTML 解释页面：包含令牌桶（token-bucket）流程图、
3-4 个关键代码片段并附注释，以及底部的"注意事项"部分。
针对只读一次的读者优化排版。
```
...
有时候纯文本框很难描述你想要的东西。在这种情况下，让 Claude 为你正在处理的确切内容构建一个一次性编辑器。不是产品，也不是可复用的工具，而是一个**单一的 HTML 文件**，专门为这一份数据量身打造。

<mark style={{backgroundColor: '#ff9900', padding: '0 4px', borderRadius: '3px'}}>关键技巧：始终以导出结束</mark> —— 一个"复制为 JSON"或"复制为 prompt"的按钮，将你在 UI 中做的任何操作转换回可以粘贴到 Claude Code 中的内容。

### 示例提示词

```
我需要重新排列这 30 个 Linear 工单的优先级。给我做一个 HTML 文件，
每个工单是一张可拖拽的卡片，分布在"立即做 / 下一步 / 之后 / 砍掉"四列中。
先按你的最佳判断预排序。添加一个"复制为 Markdown"按钮，
导出最终排序并为每个分类附上一句话理由。
```

```
这是我们的功能开关（feature flag）配置。为它构建一个表单编辑器，
按区域分组，展示它们之间的依赖关系，如果我启用了一个前置条件未开启的开关就发出警告。
添加一个"复制 diff"按钮，只给我变更的键。
```

```
我正在调优这个系统提示词。做一个并排编辑器：左边是可编辑的提示词，
变量槽位高亮显示；右边是三个示例输入，实时重新渲染填充后的模板。
添加字符/token 计数器和一个复制按钮。
```

### 适用场景

- 重新排序、分类或归类任何东西（工单、测试用例、反馈）
- 编辑结构化配置（功能开关、环境变量、有约束的 JSON/YAML）
- 调优 prompt、模板或文案并实时预览
- 数据集策展：批准/拒绝行、标记示例、导出选择
- 注释文档、转录或 diff 并导出注释
...
> 以下是 Anthropic Claude 官网的完整设计规范，当你需要让 Claude 生成与 Claude 官网一致风格的 HTML 产物时，可以将此设计系统作为参考传入。
