...
我们这次给大家讲的测试的框架呢，是 [unittest](https://docs.python.org/zh-cn/3/library/unittest.html)，是 Python 自带的一个测试框架。我本人是非常喜欢用标准库的，所以一直在用 unittest。大家先把 unittest 学明白了，以后如果你想用其他的 framework，也差不多。
...
我们举个最简单的例子。假设我们在运行每一个测试之前、之后，都要打印这个“测试开始了”，“测试结束了”。那我们可以在这个 test class 之内，通过定义 [setUp](https://docs.python.org/zh-cn/3/library/unittest.html#unittest.TestCase.setUp) 函数和 [tearDown](https://docs.python.org/zh-cn/3/library/unittest.html#unittest.TestCase.tearDown) 函数来完成。我们看，在 setUp 函数里面，我们打印了一个 start，然后在 tearDown 函数里面，我们打印了一个 end。于是我们在运行 unittest 的时候，输出里面就会有一个 start 跟一个 end。那如果我们又增加了一个 test method，就可以看到 start 跟 end 被打印了两次。
...
那如果我们想每一个 test class 运行之前和结束之后做一些事儿的话，我们用到的函数就是 [setUpClass](https://docs.python.org/zh-cn/3/library/unittest.html#unittest.TestCase.setUpClass) 跟 [tearDownClass](https://docs.python.org/zh-cn/3/library/unittest.html#unittest.TestCase.tearDownClass)。注意，这两个东西必须要用 classmethod 这个装饰器装饰一下。
...
在这个例子里面，我测试了一下，如果我 [sys.platform](https://docs.python.org/zh-cn/3/library/sys.html#sys.platform) 是 win32 的话，就不运行这个测试，也就是这个测试不在 Windows 上运行。

那还有一个比较常见的条件呢，就是 [version_info](https://docs.python.org/zh-cn/3/library/sys.html#sys.version_info)，就是这个 Python 的版本。比如这里我写的 sys.version_info 小于 3.7，就是说它只支持 3.7 及以上的版本。
...
我们可以通过控制 [unittest command line 的 argument](https://docs.python.org/zh-cn/3/library/unittest.html#command-line-interface)，来告诉 unittest 运行哪些 test。比如在这里，我们运行的是，tests 这个文件夹下的 test_vector 这个 module，里面的 test vector 这个 class 里面的 test_add 这个 method。可以看到，它只运行了一个 test。

那如果我们把条件放宽一些，我们运行 test 这个文件夹里面的 test_vector 这个 file，它就会运行两个 test 了。

<CodeWalkthrough variant="unittest" step={10} />
