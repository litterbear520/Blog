...
限制配置文件权限。官方 systemd 服务默认以 `nobody` 用户运行，在 Ubuntu 中可以让 `nogroup` 组只读配置：
...
私钥已经写入正式配置，可以清除当前 shell 中的私钥变量：
...
按 `Ctrl + C` 退出日志跟踪。不要为了排查问题把包含客户端地址或凭据的完整日志公开发布。
...
- 腾讯云防火墙只开放实际需要的端口；
- SSH 22 端口尽可能限制来源 IP；
- 客户端 YAML、URI 和 `client.env` 使用 `600` 权限；
...
~/.config/littlebear-vpn/client.env
~/.config/littlebear-vpn/littlebear-vpn.yaml
~/.config/littlebear-vpn/littlebear-vpn-uri.txt
Mac 上的 ~/.ssh/config
Mac 上的 SSH 私钥（单独加密备份）
```
