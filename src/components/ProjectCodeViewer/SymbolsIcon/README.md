# ProjectCodeViewer 的 Symbols 文件图标
...
文件树和标签栏均由共用的 ProjectCodeViewer 使用此映射，覆盖 MCP 三个演练、Python unittest 与 commerce 路线。图标不随教学模式变化。
...
```bash
node --test src/components/ProjectCodeViewer/SymbolsIcon/icons.test.mjs
npm run build
```

第一条命令只使用 Node 标准库，检查文件关联、目录关联、未知名称回退、原始 SVG 完整性和接入点。第二条命令先运行查看器回归，再检查真实 Docusaurus / React / SVG 构建。
