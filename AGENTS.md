...
- **代码块预录输出**：代码围栏加 `run`，紧接一个 `output` 围栏写运行结果，构建时由 `plugins/remark-run-output.js` 合并；页面上代码块右上角出现 ▷ 按钮，点击后底部展开输出面板逐行打印，面板右上角一个终端图标角标（与复制按钮同一列，悬停提示为预录），不单独占一行，再点收起。不是真跑代码，输出要先在本地跑一遍如实录入
